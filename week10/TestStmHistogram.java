...
public class TestStmMap {
    ...
    private static double exerciseMap(int threadCount, int perThread, int range,
        ...
        final int per = perThread, r = range;
    ...
    private static void exerciseAllMaps() {
        final int bucketCount = 100000, threadCount = 16;
        final int iterations = 1600000, perThread = iterations / threadCount;
...
class StmMap<K, V> implements OurMap<K, V> {
    ...
    public V get(K k) {
        ItemNode<K,V> node = atomic(new Callable<ItemNode<K,V>>(){
            public ItemNode<K,V> call(){
                final TxnRef<ItemNode<K,V>>[] bs = buckets.get();
                int hash = getHash(k) % bs.length;
                return bs[hash].get();
            }
        });
    ...
    public V putIfAbsent(K k, V v) {
        ...
        int hash = getHash(k) % buckets.atomicGet().length;
        ...
        if(ItemNode.search(node, k, holder)){
            ...
            bs[hash].atomicSet(new ItemNode<K,V>(k,v, node));
            size.atomicIncrementAndGet(1);
            return null;
        }
    }
    ...
    public V remove(K k) {
        ...
        ItemNode<K,V> node = bs[hash].atomicGet();
        Holder<V> holder = new Holder<V>();
        bs[hash].atomicSet(ItemNode.delete(node, k, holder));
        size.atomicIncrementAndGet(-1);
        return holder.get();
    }
    ...
    public void dump(){
        TxnRef<ItemNode<K,V>>[] bs = buckets.atomicGet();
        for(int i = 0; i < bs.length; i++){
            ItemNode<K,V> current = bs[i].atomicGet();
            while(current != null){
