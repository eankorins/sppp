...
public class TestCache {
  public static void main(String[] args) throws InterruptedException {
   ...
   Mark7("Memoizer1", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer1<Long, long[]>(f));
            }});
   Mark7("Memoizer2", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer2<Long, long[]>(f));
            }});
   Mark7("Memoizer3", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer3<Long, long[]>(f));
            }});
   Mark7("Memoizer4", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer4<Long, long[]>(f));
            }});
   Mark7("Memoizer5", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer5<Long, long[]>(f));
            }});
   Mark7("Memoizer6", new IntToDouble(){
     public double call(int i) { 
              return exerciseFactorizer(new Memoizer<Long, long[]>(f));
            }});
  }
  ...
  int threadCount = 16;
  final long start = 10_000_000_000L, range = 2000;

    Thread[] threads = new Thread[threadCount];

    for (int t=0; t < threadCount; t++) {
      final long offset = t * range/4;
      final long from = 10_000_002_000L + offset;
      final long to = 10_000_03_999L + offset;
      
      threads[t] = new Thread(new Runnable() { public void run() {
        for (long i = 10_000_000_000L; i < 10_000_001_999L; i++){
          try {
          f.compute(i);
